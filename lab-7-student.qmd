...
#| label: setup
library(tidyverse)
fish <- read_csv(file = here::here("data", 
                                      "BlackfootFish.csv"), 
...
## I used this to help me solve this question
...
There are 1796 observations with missing values. These missing values are all in the variable of weight.
...
fish |>
  ...
  summarise(missing_weight = n()) |>
  ggplot(aes(x = year, 
             y = missing_weight,
             fill = factor(trip))) +
  geom_bar(stat = "identity") +
  facet_wrap(~section) +
  labs(title = "Missing Weights of Trout Over the Years \n(Grouped by Species and Section)",
       x = "Year",
       y = NULL,
       subtitle = "Number of Missing Weights",
       fill = "Trip") +
  theme_bw()
...
In the last line, when trying to find the width of the weight, you accidentally took the maximum weight and subtracted the minimum length from that value, which doesn't give you the true width of the values in weight. 
...
rescale_01 <- function(input_vec) {
  stopifnot(is.numeric(input_vec), 
            length(input_vec) > 1)
  rescale_vec <- (input_vec - min(input_vec, 
                                  na.rm = TRUE)) / (max(input_vec, 
                                                        na.rm = TRUE) - min(input_vec, 
                                                                            na.rm = TRUE))
  return(rescale_vec)
}
```
...
#|   - "Rescaled Values of Fish Length (mm)"
fish |>  
  ggplot(aes(x = length)) + 
  ...
  labs(x = "Original Values of Fish Length (mm)",
       y = NULL,
       subtitle = "Counts",
       title = "Histogram of the Original Values of Fish Length") +
...
fish |>  
  ...
  labs(x = "Rescaled Values of Fish Length (mm)",
       y = NULL,
       subtitle = "Counts",
       title = "Histogram of the Rescaled Values of Fish Length") +
  scale_y_continuous(limits = c(0,4000))
```
